...
until one of :func:`ca.flush_io`, :func:`ca.pend_io`, :func:`ca.pend_event`, or :func:`ca.sg_block`
are called allowing several operations to be efficiently sent over the network together.


Status Codes
------------
If successful, the functions return the status code :data:`ca.ECA.NORMAL`.
...
when a response arrives. The functions :func:`ca.put`, :func:`ca.get` and :func:`ca.create_subscription`
...
The *status* field will be set to one of the CA error codes :data:`ca.ECA` and will indicate
the status of the operation performed in the IOC.
If the status field isn't set to :data:`ca.ECA.NORMAL` or data isn't normally returned from the operation (i.e. put call back)
...
To modify this behavior see :func:`ca.add_exception_event`.
...
The process variable's maximum native element count is available from :func:`ca.element_count` when the channel is connected.
...
Three methods can be used to determine if a channel is connected: the application program might call :func:`ca.state`
to obtain the current connection state, block in :func:`ca.pend_io` until the channel connects,
or install a connection callback handler when it calls :func:`ca.create_channel`.
The :func:`ca.pend_io` approach is best suited to simple command line programs with short runtime duration,
and the connection callback method is best suited to toolkit components with long runtime duration.
Use of :func:`ca.state` is appropriate only in programs that prefer to poll for connection state changes
instead of opting for asynchronous notification.
The :func:`ca.pend_io` function blocks only for channels created specifying no callback function.
The user's connection state change function will be run immediately from within :func:`ca.create_channel`
...
Each thread that calls :func:`ca.create_context` for the first time either directly or
...
The priority of auxiliary threads spawned by the CA client library are at fixed offsets from the priority of the thread that called :func:`ca.create_context`.
An application specific auxiliary thread can join a CA context by calling :func:`ca.attach_context`
using the CA context identifier that was returned from :func:`ca.current_context`
...
after destroying any channels or application specific threads that are attached to it, by calling :func:`ca.destroy_context`.
...
This requires that your application must either wait in one of :func:`ca.pend_event`, :func:`ca.pend_io`,
or :func:`ca.sg_block` or alternatively it must call :func:`ca.poll` at least every 100 milli-seconds.
